...
Proc OilgenIntegration::runOilgenTarget(OilgenOpts opts,
  ...
  std::string generatorExe =
      std::string(OILGEN_EXE_PATH) + " " + path.native() + " ";
  if (auto prefix = writeCustomConfig("prefix", configPrefix)) {
  ...
  std::string compilerExe =
      std::string(CXX) +
      " a.o input_src.cpp "
      "/data/users/jakehillion/object-introspection-sl/oi/exporters/Json.cpp "
      "/data/users/jakehillion/object-introspection-sl/oi/"
      "IntrospectionResult.cpp "
      "/data/users/jakehillion/object-introspection-sl/oi/exporters/"
      "ParsedData.cpp";
  for (const auto& arg : clangArgs) {
    compilerExe += ' ';
    compilerExe += arg;
