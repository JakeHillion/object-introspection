...
Proc OilgenIntegration::runOilgenTarget(OilgenOpts opts,
  ...
  auto srcPath = workingDir / "input_src.cpp";
  {
    std::ofstream file{srcPath, std::ios_base::app};
    file << opts.targetSrc;
  }

  std::string generatorExe =
      std::string(OILGEN_EXE_PATH) + " " + srcPath.native() + " --output=oil_generated.o --debug-level=3 ";
  if (auto prefix = writeCustomConfig("prefix", configPrefix)) {
  ...
  // TODO: get this from the CMake arguments for integration_test_target.cpp
  // somehow
  // clang-format off
  std::vector<std::string> clangArgs{
      ...
      "/usr/include",
      "-isystem",
      "/data/users/jakehillion/object-introspection-sl/build/_deps/folly-src",
  ...
  std::string compilerExe =
      std::string(CXX) +
      " input_src.cpp oil_generated.o -o generated_target "
      "/data/users/jakehillion/object-introspection-sl/oi/IntrospectionResult.cpp "
      "/data/users/jakehillion/object-introspection-sl/oi/exporters/ParsedData.cpp ";
  for (const auto& arg : clangArgs) {
    compilerExe += ' ';
    compilerExe += arg;
  }
  ...
  std::string targetExe = "./generated_target";
  if (verbose) {
    std::cerr << "Running: " << targetExe << std::endl;
  }
