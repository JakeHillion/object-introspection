...
template <typename Res>
SizedResult<Res>::const_iterator::const_iterator(It it, const It& end) : data_{it} {
  struct StackEntry {
    size_t index;
    size_t depth;
  };
...
SizedResult<Res>::const_iterator::operator++() {
  // The below iterator is already pointing at the first element while this iterator is not. Skip incrementing it the first time around.
  if (count_ != 0)
    ++data_;
...
SizedResult<Res>::const_iterator::operator*() const {
  // auto* leak = new SizedResult<Res>::Element{*data_, (size_t)-1};
  // return *leak;
  return *next_;
}
template <typename Res>
...
SizedElement<El>::SizedElement(const El& el, size_t size_) : El{el}, size{size_} {
}
