...
class ClangTypeParser {
 public:
  ClangTypeParser(TypeGraph& typeGraph, const std::vector<std::unique_ptr<ContainerInfo>>& containers, ClangTypeParserOptions options)
 ...
 private:
  ...
  std::optional<TemplateParam> enumerateTemplateParam(const clang::TemplateArgument&);
  std::optional<TemplateParam> enumerateTemplateTemplateParam(const clang::TemplateName&);
